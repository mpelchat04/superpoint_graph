...
class PointNet(nn.Module):
    ...
    """
    def __init__(self, nf_conv, nf_fc, nf_conv_stn, nf_fc_stn, nfeat, nfeat_stn=2, nfeat_global=1, prelast_do=0.5, last_ac=False, is_res=False,
                 norm='batch', affine=True, n_group=1, last_bn=False):
...
class CloudEmbedder():
    """ Evaluates PointNet on superpoints. Too small superpoints are assigned zero embeddings.
        Can optionally apply memory mongering (https://arxiv.org/pdf/1604.06174.pdf) to decrease memory usage.
    """
...
class LocalCloudEmbedder:
    ...
    def run_batch(self, model, clouds, clouds_global, *excess):
        ...
        for i in range(1, n_batches+1):
            out = torch.cat((out, model.ptn(clouds[i * batch_size:(i+1) * batch_size, :, :],
                                            clouds_global[i * batch_size:(i+1) * batch_size, :])))
    ...
    def run_batch_cpu(self, model, clouds, clouds_global, *excess):
        ...
        for i in range(1,n_batches+1):
            emb = self.run_batch(model, clouds[i * batch_size:(i+1) * batch_size, :, :],
                                 clouds_global[i * batch_size:(i+1) * batch_size, :])
