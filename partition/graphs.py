...
def compute_graph_nn_2(xyz, k_nn1, k_nn2, voronoi=0.0):
    ...
    if voronoi > 0:
        ...
        graph["source"] = np.hstack((tri.vertices[:, 0], tri.vertices[:, 0],
                                     ...
                                     tri.vertices[:, 1], tri.vertices[:, 2])).astype('uint64')
        graph["target"] = np.hstack((tri.vertices[:, 1], tri.vertices[:, 2],
                                     tri.vertices[:, 3], tri.vertices[:, 2],
                                     tri.vertices[:, 3], tri.vertices[:, 3])).astype('uint64')
        ...
        graph["source"] = np.hstack((graph["source"], np.matlib.repmat(range(0, n_ver), k_nn1, 1).flatten(order='F').astype('uint32')))
