...
def main():
    ...
    parser.add_argument('--use_voronoi', default=0.0, type=float, help='uses the Voronoi graph in combination to knn to build the adjacency graph, '
    ...
    for folder in folders:
        ...
        for file in files:
            ...
            if args.dataset == 's3dis':
                data_file = f"{data_folder}{file_name}/{file_name}.txt"
                str_file = f"{str_folder}{file_name}.h5"
            elif args.dataset == 'sema3d':
                file_name_short = '_'.join(file_name.split('_')[:2])
                data_file = f"{data_folder}{file_name}.txt"
                label_file = f"{data_folder}{file_name}.labels"
                str_file = f"{str_folder}{file_name_short}.h5"
            elif args.dataset == 'vkitti':
                data_file = f"{data_folder}{file_name}.npy"
                str_file = f"{str_folder}{file_name}.h5"
            i_file = i_file + 1
            ...
            if os.path.isfile(str_file):
                ...
                if args.dataset == 's3dis':
                    ...
                    if pruning:
                        xyz, rgb, labels, o = libply_c.prune(xyz.astype('f4'), args.voxel_width, rgb.astype('uint8'), labels.astype('uint8'),
                ...
                if args.dataset == 's3dis':
                    ...
                    dump, objects = libcp.cutpursuit2(np.array(hard_labels).reshape((n_ver, 1)).astype('f4'), edg_source, edg_target, edge_weight,
                                                      node_weight, 0.01)
                    is_transition = objects[graph_nn["source"]] != objects[graph_nn["target"]]
                elif args.dataset == 'vkitti':
                    ...
                    dump, objects = libply_c.connected_comp(n_ver, graph_nn["source"].astype('uint32'), graph_nn["target"].astype('uint32'),
...
def get_sema3d_info():
    ...
    return {
        'classes': 8,
        'inv_class_map': {0: 'road', 1: 'grass', 2: 'tree', 3: 'bush', 4: 'building',
                          5: 'hardscape', 6: 'artifacts', 7: 'car', 8: 'chair'},
    }
...
def get_vkitti_info():
    ...
    return {
        'classes': 13,
        'inv_class_map': {0: 'Terrain', 1: 'Tree', 2: 'Vegetation', 3: 'Building', 4: 'Road', 5: 'GuardRail', 6: 'TrafficSign',
                          7: 'TrafficLight', 8: 'Pole', 9: 'Misc', 10: 'Truck', 11: 'Car', 12: 'Van', 13: 'None'},
    }
    
                
def create_s3dis_datasets(args):
...
def create_vkitti_datasets(args):
    """ Gets training and test datasets. """
    # Load formatted clouds
    testlist, trainlist = [], []
    for n in range(1, 7):
...
def create_sema3d_datasets(args):
    """ Gets training and test datasets. """
    
    train_names = ['bildstein_station1', 'bildstein_station5', 'domfountain_station1', 'domfountain_station3', 'neugasse_station1', 'sg27_station1',
                   'sg27_station2', 'sg27_station5', 'sg27_station9', 'sg28_station4', 'untermaederbrunnen_station1']
...
def graph_loader(entry, train, args, db_path, test_seed_offset=0, full_cpu = False):
    ...
    xyz, rgb, edg_source, edg_target, is_transition, local_geometry, labels, objects, elevation, xyn = read_structure(entry, 'geof' in args.ver_value)
...
def write_embeddings(file_name, args, embeddings, edge_weight=[]):
    ...
    if len(embeddings) > 0 and not 'embeddings' in data_file:
        data_file.create_dataset('embeddings', data=embeddings, dtype='float32')
