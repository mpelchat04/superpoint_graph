...
from supervized_partition.graph_processing_custom import *
...
def parse_args():
    ...
    # Point net
    parser.add_argument('--ptn_embedding', default='ptn', help='configuration of the learned cloud emebdder (ptn): '
                                                               'uses PointNets for vertices embeddings. no other options so far :)')
    ...
    # Loss
    parser.add_argument('--loss_weight', default='crosspartition', help='[none, proportional, sqrt, seal, crosspartition] '
                                                                        'which loss weighting scheme to choose to train the model. unweighted: '
                                                                        'use classic cross_entropy loss, proportional: '
                                                                        'weight inversely by transition count,  '
                                                                        'SEAL: use SEAL loss as proposed in '
                                                                        'http://jankautz.com/publications/LearningSuperpixels_CVPR2018.pdf, '
                                                                        'crosspartition : our crosspartition weighting scheme')
    parser.add_argument('--loss', default='TVH_zhang', help='Structure of the loss : '
                                                            'first term for intra edge (chose from : tv, laplacian, TVH (pseudo-huber)), '
                                                            'second one for interedge (chose from: zhang, scad, tv)')
    ...
    # Graph-Clustering
    parser.add_argument('--ver_value', default='ptn', help='what value to use for vertices (ptn): uses PointNets, '
                                                           '(geof) : uses geometric features, (xyz) uses position, (rgb) uses color')
    ...
    parser.add_argument('--CP_cutoff', default=10, type=int, help='Minimum accepted component size in cut pursuit. '
                                                                  'if negative, chose with respect tot his number and the reg_strength '
                                                                  'as explained in the paper')
    parser.add_argument('--spatial_emb', default=0.2, type=float, help='Weight of xyz in the spatial embedding. When 0 : no xyz')
    parser.add_argument('--edge_weight_threshold', default=-0.5, type=float, help='Edge weight value when diff>1. if negative, '
                                                                                  'then switch to weight = exp(-diff * edge_weight_threshold)')

    # Metrics
    parser.add_argument('--BR_tolerance', default=1, type=int, help='How far an edge must be from an actual transition '
...
def dataset(args):
    ...
    if args.dataset == 'airborne_lidar':
        dbinfo = get_airborne_lidar_info()
        create_dataset = create_airborne_lidar_datasets
    else:
...
def embed(args):
    ...
    def train(i_epoch):
        ...
        model.train()
        loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, collate_fn=graph_collate, num_workers=args.nworkers,
        ...
        with torch.no_grad():
            ...
            for bidx, (fname, edg_source, edg_target, is_transition, labels, objects, clouds_data, xyz) in enumerate(loader):
                ...
                if len(is_transition) > 1:
                    ...
                    confusion_matrix_BR.count_predicted_batch_hard(is_transition, relax_edge_binary(pred_transition, edg_source, edg_target,
                                                                                                    xyz.shape[0], args.BR_tolerance).astype('uint8'))
                    confusion_matrix_BP.count_predicted_batch_hard(relax_edge_binary(is_transition, edg_source, edg_target,
                                                                                     xyz.shape[0], args.BR_tolerance),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.test_nth_epoch == 0:  # or epoch+1==args.epochs:
            ...
            print(f"-> Train loss: {loss} - Test Loss: {loss_test}  |  n_clusters:  {n_clusters_test}  |  "
                  f"Test ASA: {ASA_test}  |  Test BR: {BR_test}  |  Test BP : {BP_test}")
        else:
            loss_test, n_clusters_test, ASA_test, BR_test, BP_test = 0, 0, 0, 0, 0
            print(f"-> Train loss: {loss}  superpoints size : {n_sp}")
...
def create_model(args):
    ...
    if args.learned_embeddings and 'ptn' in args.ptn_embedding:
        ...
        model.ptn = PointNet(args.ptn_widths[0], args.ptn_widths[1], [], [], n_feat, 0, prelast_do=args.ptn_prelast_do, nfeat_global=nfeats_global,
                             norm=args.ptn_norm, is_res=False, last_bn=True)  # = args.normalize_intermediary==0)
